...
pause(5)
    for t=1:25       %start
        if (t>-1) && (t<6)
            centre_pelvis(1)=3;       %upwards movement
            centre_pelvis(2)=3*t/5+5; %sidewards movement
            xa=3;
            height_ankle = 0;
            centre_pelvis(3)= h*cos(atan((centre_pelvis(2)-5)/h))+1;  %forwad movement
            right_hand_angles(1)= -3*pi/180;   %right shoulder angle
            right_hand_angles(3)= 16*pi/180;   %right elbow movement
            left_hand_angles(1)= -3*pi/180;   %left shoulder angle
            left_hand_angles(3)= 16*pi/180;   %left elbow movement
        elseif (t>5) && (t<11)
            centre_pelvis(1)=3;
            centre_pelvis(2)=8;
            xa=3;
        ...
        plot2_final(left_leg,right_leg,waist,right_arm,left_arm);  %plots a line joining 3 points of right leg, 3 on left leg ,3 on right arm, 3 on left arm separately
...
for m=1:no_of_steps
    for t=1:40    %left leg as swing one
        if (t>-1) && (t<11)       %dsp1
            centre_pelvis(1) = t/5;            %upwards movement
            centre_pelvis(2) = -3*(t)/10+5;    %sidewards movement
            xa = -3;
            height_ankle = 0;
            centre_pelvis(3) =h*cos(atan((centre_pelvis(2)-5)/h))+1;  %forwad movement
